...
<script>
  const INPUT_ICON = "bmlocal://assets/search.png";
  const CLOSE_ICON = "bmlocal://assets/search_close.png";
  const ARROW_ICON = "bmlocal://assets/search_arrow.png";
  ...
  export default {
    ...
    computed: {
      ...
      inputStyle() {
        const { barStyle } = this;
        if (barStyle.width) {
          return { width: this.needShowCancel ? barStyle.width - 126 : barStyle.width - 40 }
        }
        return { width: this.needShowCancel ? '624px' : '710px' }
      }
    },
    data: () => ({
